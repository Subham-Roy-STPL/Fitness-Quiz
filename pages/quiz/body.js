    ...
    return (
        <div className='relative h-screen'>
            ...
            <div className='relative top-20 left-0 w-screen bg-[#dedccf] min-h-[90%] z-10 flex flex-col justify-center items-center'>
                ...
                <input value={weight} onChange={(e) => { setWeight(e.target.value) }} className='border border-black text-black min-[510px]:w-[500px] w-[300px] text-center py-2 m-5 rounded cursor-pointer focus:bg-white focus:border-none bg-none min-[510px]:text-2xl text-lg font-extralight' />
                <p className='text-gray-800'>Height {opt == "Metric" ? "(cm)" : "(feet)"}</p>
                <input value={height} onChange={(e) => { setHeight(e.target.value) }} className='border border-black text-black min-[510px]:w-[500px] w-[300px] text-center py-2 m-5 rounded cursor-pointer focus:bg-white focus:border-none bg-none min-[510px]:text-2xl text-lg font-extralight' />
            </div>
